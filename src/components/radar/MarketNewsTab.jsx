...
export default function MarketNewsTab({ articles }) {
  if (!articles || articles.length === 0) {
    return <p className="text-[#666] text-sm py-4">No market news available</p>;
  }
        ...
        return (
          ...
          >
            ...
            <div className="flex justify-between mt-1 text-xs text-[#666]">
              ...
              <span>{item.pubDate ? timeAgo(item.pubDate) : item.publishedAt ? timeAgo(item.publishedAt) : ''}</span>
            </div>
          </a>
        );
      })}
